...
export async function getProjectMeta(projectId) {
    const res = await fetch(
        `https://ampmod.vercel.app/internalapi/projects/${projectId}`,
        { credentials: "include" }
    );
...
export async function saveProject(vm, projectId, title) {
    const body = vm.toJSON();
    const res = await fetch(
        `https://ampmod.vercel.app/internalapi/projects/update?pID=${projectId}&title=${encodeURIComponent(title || "Untitled Project")}`,
        {
            method: "POST",
            credentials: "include",
            headers: { "Content-Type": "application/json" },
